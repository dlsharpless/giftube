            ...
            for (i = 0; i < numberOfRecords; i++) {
                ...
                (function() {
                    ...
                    if (response.data[i].nsfw || response.data[i].looping || response.data[i].mp4) {
                        ...
                        return;
                    } else { // Build image elements to hold memes from the ajax response
                        ...
                        if (Object.keys(response.data[i]).includes("images")) {
                            ...
                            if (response.data[i].images[0].nsfw || response.data[i].images[0].looping || response.data[i].images[0].mp4) {
                                // Skip the result and look for an additional one from the response if nsfw
                                if (response.data.length > numberOfRecords) {
                                    numberOfRecords++;
                                }
